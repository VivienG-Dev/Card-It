...
function useRegister() {
  ...
  return { email, password, username, register, successfulResponse, errorResponse };
}
          ...
          <div v-if="isMenuOpen" class="absolute -right-3 mt-3 w-48 rounded-md shadow-lg bg-white z-50">
            <div v-if="authStore.isAuthenticated" class="py-1">
              ...
              <button @click="authStore.logout" class="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100"
                @click.native="isMenuOpen = false">Logout</button>
            </div>
          </div>
        </div>
      </ClientOnly>
        ...
        <div class="bg-white rounded-lg shadow-lg py-3 px-14 w-[500px] h-[500px] mx-auto relative">
          ...
          <div v-if="showRegisterForm" class="flex flex-col">
            ...
            <form @submit.prevent="register" class="space-y-4">
              <div>
                ...
                <input type="text" placeholder="Username" name="Username" ref="usernameRef" v-model="usernameRegister"
                  @input="validateUsername(usernameRegister)"
                  class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :class="{ 'border-red-500': usernameError || (errorResponseRegister && usernameRegister === '') }" />
                <div class="text-red-500 text-xs text-center h-2">{{ usernameError }}</div>
              </div>
              <div>
                <label for="Email" class="font-normal text-sm">Email:</label>
                <input type="email" placeholder="Email" name="Email" v-model="emailRegister"
                  @input="validateEmail(emailRegister)"
                  class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :class="{ 'border-red-500': emailError || (errorResponseRegister && emailRegister === '') }" />
              ...
              <div>
                ...
                <div class="relative flex items-center">
                  <input id="Password" :type="isPasswordVisible ? 'text' : 'password'" placeholder="Password"
                    v-model="passwordRegister" @input="validatePassword(passwordRegister)"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-red-500': passwordErrorRegister || (errorResponseRegister && passwordRegister === '') }" />
                  <Icons @click="togglePasswordVisibility()"
              ...
              <p class="text-sm text-center font-extralight">
                Already have an account? <NuxtLink to="/?modal=login" class="underline underline-offset-2">Sign-In
                </NuxtLink>
              </p>
            </form>
          </div>
          ...
          <div v-else class="space-y-4">
            ...
            <div class="flex justify-center items-center w-full h-10 rounded-lg text-sm"
              :class="{ 'bg-red-100 text-red-500': errorResponseSignIn, 'bg-green-100 text-green-500': successfulResponseSignIn }">
              {{ errorResponseSignIn || successfulResponseSignIn }}
            </div>
            <form @submit.prevent="signIn" class="space-y-4">
              <div>
                <label for="Email">Email:</label>
                <input type="email" placeholder="Email" ref="emailRef" v-model="emailSignIn"
                  @input="validateEmail(emailSignIn)"
                  class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :class="{ 'border-red-500': emailError || (errorResponseSignIn && emailSignIn === '') }" />
              ...
              <div>
                ...
                <div class="relative flex items-center">
                  <input id="Password" :type="isPasswordVisible ? 'text' : 'password'" placeholder="Password"
                    v-model="passwordSignIn" @input="validatePassword(passwordSignIn)"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-red-500': passwordError || (errorResponseSignIn && passwordSignIn === '') }" />
                  <Icons @click="togglePasswordVisibility()"
                    svgClass="w-6 h-6 absolute right-2 top-1/2 transform -translate-y-1/2"
                    :isType="isPasswordVisible ? 'eye' : 'eyeSlash'" />
                </div>
              ...
              <p class="text-sm text-center font-extralight">No account? <NuxtLink to="/?modal=register"
                  class="underline underline-offset-2">Register</NuxtLink>
              </p>
              <p class="text-sm text-center font-extralight">
